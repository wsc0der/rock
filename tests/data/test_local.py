...
class TestDatabase(unittest.TestCase):
    ...
    def setUp(self):
        local.init()
        self.connection = local.get_connection()
        return super().setUp()
    ...
    def tearDown(self):
        ...
        if os.path.exists(local.DB_PATH):
            os.remove(local.DB_PATH)
            os.rmdir(os.path.dirname(local.DB_PATH))
        return super().tearDown()

    def test_insert_exchange(self):
        """Test inserting an exchange into the database."""
        # Insert an exchange
        local.insert_exchange('Shanghai Stock Exchange', 'SSE', 'stock')

        # Check if the exchange was inserted successfully
        cursor = self.connection.cursor()
        cursor.execute(f"SELECT * FROM {local.Tables.EXCHANGE} WHERE acronym='SSE';")
    ...
    def test_insert_security(self):
        ...
        # Insert a security
        local.insert_security('000001', 'Ping An Bank', 'stock', 1)

        # Check if the security was inserted successfully
        cursor = self.connection.cursor()
        cursor.execute(f"SELECT * FROM {local.Tables.SECURITY} WHERE symbol='000001';")
    ...
    def test_insert_history(self):
        ...
        for case in valid_cases:
            local.insert_history(*case)

        # Check if the history was inserted successfully
        cursor = self.connection.cursor()
        timestamp = dt.fromisoformat(valid_cases[0][1]).timestamp()
        cursor.execute(f"SELECT * FROM {local.Tables.HISTORY} WHERE datetime={timestamp};")
        history = cursor.fetchone()
        cursor.execute(f'SELECT history_updated_at FROM {local.Tables.SECURITY} WHERE id={valid_cases[0][0]};')
    ...
    def test_bulk_insert_history(self):
        ...
        local.bulk_insert_history(histories)

        # Check if the histories were inserted successfully
        cursor = self.connection.cursor()
        cursor.execute(f'''
            SELECT * FROM {local.Tables.HISTORY} WHERE datetime={dt.fromisoformat('2025-03-01').timestamp()};
        ''')
        history_1 = cursor.fetchone()

        cursor.execute(f'''
            SELECT * FROM {local.Tables.HISTORY} WHERE datetime={dt.fromisoformat('2025-03-02').timestamp()};
        ''')
        ...
        cursor.execute(f'SELECT history_updated_at FROM {local.Tables.SECURITY} WHERE id={histories[0][0]};')
        history_updated_at = cursor.fetchone()
        self.assertTrue(history_updated_at[0] <= dt.now(), msg="Security history_updated_at should be updated.")
        ...
        with self.assertRaises(IntegrityError):
            local.bulk_insert_history(invalid_case)

        # Check if the database is still consistent
        cursor.execute(f"SELECT * FROM {local.Tables.HISTORY} WHERE datetime='2025-03-03';")
        ...
        with self.assertRaises(ValueError):
            local.bulk_insert_history(invalid_datetime)
        cursor.close()
    ...
    def test_get_security(self):
        ...
        for s in securities:
            local.insert_security(*s)

        # Test single case
        test_security = securities[0]
        result = local.get_security(test_security[0])
        ...
        # Test multiple cases
        result = local.get_security([item[0] for item in securities])
        self.assertTrue(isinstance(result, Sequence), "Security should be returned as a Sequence object.")
        ...
        results = local.get_security(invalid_cases)
        ...
        results = local.get_security(partial_cases)
